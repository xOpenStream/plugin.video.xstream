...
def _getAvailableRAM():
    """Liest MemAvailable aus /proc/meminfo. Gibt MB zurueck oder None bei Fehler."""
    try:
...
_SMARTTUBE_MIN_RAM_MB = 220  # Minimum freies RAM fuer SmartTube (sonst YT-Addon)
...
def _showHintIfNeeded(has_yt_player, has_own_key, found_any, played_imdb, primary_lang='de'):
    ...
    Popup: has player, no own key, zero hits -> suggest YT addon with own API key.
    YouTube install is handled by pre-check dialog in playTrailer().
    ...
    try:
        ...
        if has_yt_player and not has_own_key and not found_any:
            # Zero hits, no own key -> suggest YT addon with API key
...
def playTrailer(tmdb_id, mediatype='movie', title='', year='', poster='', pref_lang='de', season=None):
    ...
    # ── Pre-check: offer YouTube install if no player and user wants non-EN trailers ──
    if not has_yt_player:
        primary_lang = languages[0] if languages else pref_lang
        if primary_lang != 'en':
            ...
            if not _win.getProperty(_yt_asked):
                ...
                if is_de_gui:
                    if is_android:
                        msg = ('Ohne YouTube Add-on oder SmartTube sind nur englische Trailer (IMDB) verf\u00fcgbar.\n'
                               'YouTube Add-on installieren f\u00fcr Trailer in deiner Sprache?\n'
                               '(SmartTube ist eine externe Android-App und muss manuell von GitHub installiert werden.)')
                    else:
                        msg = ('Ohne YouTube Add-on sind nur englische Trailer (IMDB) verf\u00fcgbar.\n'
                               'YouTube Add-on installieren f\u00fcr Trailer in deiner Sprache?')
                else:
                    if is_android:
                        msg = ('Without YouTube add-on or SmartTube only English trailers (IMDB) are available.\n'
                               'Install YouTube add-on for trailers in your language?\n'
                               '(SmartTube is an external Android app and must be installed manually from GitHub.)')
                    else:
                        msg = ('Without YouTube add-on only English trailers (IMDB) are available.\n'
                               'Install YouTube add-on for trailers in your language?')
